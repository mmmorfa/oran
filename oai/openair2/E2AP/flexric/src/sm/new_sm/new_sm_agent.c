 ...
 sm_ctrl_out_data_t on_control_new_sm_ag(sm_agent_t const* sm_agent, sm_ctrl_req_data_t const* data)
{
  ...
  new_ctrl_req_data_t new_ctrl = {0};

  new_ctrl.hdr.dummy = 0; 
  new_ctrl.msg.action = msg.action;

  sm->base.io.write_ctrl(&new_ctrl);
...
sm_e2_setup_data_t on_e2_setup_new_sm_ag(sm_agent_t const* sm_agent)
{
  ...
  /*
  // RAN Function
  setup.rf.def = cp_str_to_ba(SM_NEW_SHORT_NAME);
